# R Workshop Intermediate R 
# Nov 2021 updated Nov 2024
# author: katya
# R version 4.1.3 "One Push-up"

# Set working directory to the file location (only RStudio), see basicStatsNov2024.R if this does not work for you
setwd(dirname(rstudioapi::getActiveDocumentContext()$path))

# Project-oriented workflow recommended; all files (code, dependencies, outputs) related to a single project 
...
#################################
# Part 1. Data wrangling and an introduction to tidyverse

# install_load("tidyverse")  # or load separately (my preference, above)
# browseURL("https://r4ds.hadley.nz/") # free textbook; also https://tidyverse.org

# tips from basicStatsNov2024.R: 
# only ever generate tidy data, but if data came to you in a non-tidy format, do not re-wrangle manually 
# any new dataset should be thoroughly examined before analysis
...
# But long pipes can be difficult to test and troubleshoot
...
dat <- dat %>% # use a piping operator and mutate to create another column change from previous
  mutate(z = x - lag(x)) # why did I not create a new object for testing? mutate() appends

# Mutate with grouping can be very useful
dat$id2 <- paste(c("spring", "fall")) # create a grouping ID, NB short vector is recycled here
...
mtcarsSum <- mtcars %>% # will yield deprecated warning
  ...
  summarize_each(funs(mean(., na.rm=T), n = sum(!is.na(.)), sd = sd(., na.rm = T))) # summarize mean, n, sd
...
# Optional: work through R for Data Science https://r4ds.hadley.nz/ (we will cover parts in the tidyverse workshop)
...
# Footnote 1: Reproducible workflows and coding best practices:
# Do not touch the source files, ever
# Annotate, unit test, include all dependencies (especially if asking for help)
# DRY: do not repeat yourself (NB not always easiest to follow)
#	Do one thing well: clean, simple, efficient, modular, readable (credit: UNIX philosophy; S. Ilyushkin)
# Next level: configure permanent environment (credit: N. Dunn, https://pages.github.umn.edu/dunn0404/software-management-tutorial/)
...
# A very useful example to keep on hand, regex is the gnarliest code you will write
...
# Footnote 2: Programmer's honor code, acknowledgements, use of LLMs in coding
# Acknowledge: attribute, cite, invite

# LLMs: If you cannot ask the right question, how can you tell if you are getting the right answer? 
# Trained without "remuneration or consent"; can take chunks of protected code; banned by StackOverflow;
# "Are you comfortable giving credit to LLMs?" In this case, it is a trick question!
...
# Which one is better? colNames from base is quicker, better optimized
colMeans(mtcars)
...
# Footnote 3: peer-reviewed coding help, welcome to the University of StackOverflow
...
#   labs(x = NULL)
# 
# # We want to use a function factory that inputs the desired number of bins 
...
# browseURL("https://adv-r.hadley.nz/debugging.html") # additional resources for debugging
...
plot(mod) 
text(mod, use.n = TRUE, all = TRUE, cex = 0.8) # not great, use rpart.plot to improve rpart plotting
...
# Footnote 4: And that's why you do not want to nest
# Footnote 5: AI is a tool, not a destination
...
# Additional resources: https://www.r-graph-gallery.com/, https://ggplot2-book.org/ 
...
# don't be afraid to spend a ton of time on polishing up your figures
# browse high-IF journals for incredible graphical examples, try to reproduce them
# any existing figure can be generated (if it is not hand-drawn), nothing is impossible

# Discuss philosophy of ggplot, what is advanced?

# 6.1. 3D and Heatmaps (very useful to avoid overplotting)
x1 <- rnorm(10000,11,.5)
x2 <- rnorm(10000,12,5)
...
# 6.2. Scatterplot with histograms, correlograms
...
ggcorrplot(corr, hc.order = TRUE, 
           ...
           lab_size = 3, 
           method="circle", 
           colors = c("#40E0D0", "white", "purple"), # NB this takes mixed numeric and verbal arguments
           title="Correlogram of mtcars", 
           ggtheme=theme_bw)

# 6.3. Revisit example from basicStatsNov2024.R
...
# 6.4. Bootstrapped function calculates smoothers from 1000 bootstraps followed by calculation of 
...
# Footnote 6: As an editor, what do I do when I get a manuscript? Scan the figures. Poor figures trigger
# more thorough pre-review, increase likelihood of rejection without review. What would you look at next?

# cool tricks? latest insights?
