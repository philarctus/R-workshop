...
# Return the index or value of the matched string
grep(pattern = "percent\\%", x = c("percent%","percent"), value = T)
...
# text <- c("My email address is philarctus@gmail.com,my email is dyum@hotmail.com; my name is...")
# unlist(regmatches(x = text, gregexpr(pattern = "[[:alnum:]]+\\@[[:alpha:]]+\\.com", text = text)))  
...
# Task 2. a) extract first part of the string (c("ALS1:E01", "M2:E002", "773R46:E0034"); 
# b) extract digits from "total volume (measured at the low water datum) is 22,671 cubic km"
...
f1 <- function(dat) {
  pdf("rfPlot.pdf", width = 13, height = 9)
  labels <-unique(iris$Species)
  for (i in labels){
    dataset <- iris[iris$Species == i,] 
...
# E. thematic: auto theming R plots in Shiny (via CSS), # R Markdown (via bootstraplib), and RStudio
# remotes::install_github("rstudio/thematic")
install_load("thematic")
dat <- read.csv("fishFdGl.csv")
thematic_on() # turn on thematic auto styling
ggplot(dat, aes(x = Rich, y = RaoQ, col = FRic)) +  
  geom_point(alpha = 0.7, size = 2) + 
  xlab("Fish species richness") + ylab("Functional diversity")
thematic_off()
