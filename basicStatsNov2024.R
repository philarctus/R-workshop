...
# you can use any name for your object but it cannot start with neither a number nor a symbol

# Examine object structure, note factors, characters, numeric, ensure that dates are read in correct format
# Most error codes in the beginning will result from incorrect data structure, see examples
# browseURL("https://doi.org/10.4033/iee.2013.6b.6.f") # this will open a browser window
# https://vita.had.co.nz/papers/tidy-data.pdf # R Studio also hyperlinks directly

# tip: only ever generate tidy data, it is not an option 
# but if data came to you in a non-tidy format, do not re-wrangle manually (and that requires advanced magic)
...
colnames(dat) <- make.names(colnames(dat)) # can use this to fix syntactically incorrect column names; these are ok
colnames(dat) # so nothing happens, names still the same

# Coding philosophy tip: any new dataset should be thoroughly examined before diving in to analyze it, structure and EDA

###################################
# Part 2. Regression (and its non-linear flavors, transformations, custom figures and intro to debugging)

# Simple linear regression
fit <- lm(Rich ~ FRic, data = dat) # fit a linear model with a base function lm()
# examine fit object in the environment: what do you see?
summary(fit) # summarize model output

x <- summary(fit) # examine the summary object
x$r.squared # model coefficients can be extracted, you do not need to remember this, only that it is possible

# Stat philosophy tip: analyses are easy, any existing analysis can be replicated, assumptions are the tricky part
...
# NB: you need to interact with the console window below

# Simple plot of the relationship, from base graphics
plot(Rich ~ FRic, data = dat)
plot(dat$Rich ~ dat$FRic) # an alternative not requiring specifying the dataset
plot(Rich ~ FRic) # but this won't work unless you define those objects beforehand!

# Log-transformation can be added directly to the plotting function
plot(Rich ~ log(FRic), data = dat) # note that, unlike Excel, default log is base e not 10
# also note how transformation makes data distribution "worse" in this case
...
# tip: comment out parts of your code which can over-write outputs
...
# Optional: explore Generalized Additive Models, Quantile regression
# Optional: explore highest-ranked questions/answers on CV https://stats.stackexchange.com/questions/tagged/regression
...
p <- ggplot(dat, aes(x = Rich, y = RaoQ, col = FRic)) +  #define aesthetics (x, y)
  ...
  geom_smooth(method = "loess", se = TRUE, linewidth = 0.8, alpha = 0.1, col = "black") + # model type/transparency/color
...
# tip: keep modifying arguments to see what changes (or read documentation!)

show_col(viridis_pal()(10))
show_col(viridis_pal(option = "inferno")(20))

# tip: do not use Excel for plotting, especially not default colors, it looks unprofessional
# ok to use default ggplot colors but they are not optimized for color-blindness and printing and it looks a bit lazy

# ggplot textbook https://ggplot2-book.org/ 
# Discuss philosophy of ggplot, what is advanced?
# Optional: find a figure you would like to recreate
...
str(pred)
summary(as.factor(pred$lake))
summary(as.factor(pred$TP))

full <- join(dat, pred, by = "Site", type = "left") # joining column name must be exactly equivalent 

# Optional: play with join types, these are very useful, 
# More relational data from hadley here https://r4ds.had.co.nz/relational-data.html

# One-way ANOVA with Tukey pairwise comparisons
fit <- aov(Rich ~ lake, dat = full) # apply aov() function to joined data
# NB how aov() assumes lake is a factor, this is actually very advanced magic
summary(fit)
TukeyHSD(fit) # examine which contrasts are significant

# Two-way ANOVA with an interaction effect
fit <- aov(Rich ~ lake + TP + lake:TP, dat = full)
summary(fit) # NB fit object is over-written
# stats tip: for higher-order models, interpret interactions before you interpret main effects
...
# Create a new quantitative column based on a factor via ifelse statement
full$concentration <- ifelse(full$TP == "low", 0.5, 3) # for each row, if TP is "low" assign value 0.5, else assign 3
summary(full$concentration)

## ANOVA for each factor level, intro to loops and saving output to a text file

# sink('analysis-output.txt', append=TRUE) # this will output results directly to file
labels <- unique(full$lake) # create levels for looping

for (i in labels){
  # dataset <- full[full$lake == "Erie",]  # troubleshoot loops by setting i to a value (and don't run the for line)
...
###################################
# Part 4. Introduction to ordination (ln. 194)
# Most commonly used to summarize species or environmental data in fewer dimensions

dat2 <- read.csv("species.csv") # read in the dataset
spp <- dat2[, 2:66] # subset dataframe to exclude ID and predictors
# tip: this is a poor practice to subset by column location, we can do better
spp <- subset(dat2, select = -c(plotYear, year, trt, elev))
env <- subset(dat2, select = c(year, trt, elev)) # subset dataframe to environmental predictors
...
fit <- metaMDS(spp2, k = 3, maxtry = 40, autotransform = FALSE) # increasing dimensionality allows to find a solution
fit
...
# Let's bring in treatment/predictor variables and make a more complex graph
trt <- env$trt # this could have been defined directly in the plotting function actually
